...
export function ExecutiveSummary({
  ...
  const filteredCallList = useMemo(() => {
    ...
    if (tierFilter !== "all") {
      list = list.filter((c) => c.leadTier === tierFilter);
    }
    if (callSearch.trim()) {
          ...
          <CardContent>
            {pipelineQuality.length === 0 ? (
              ...
              >
                <PieChart>
                  <ChartTooltip
                    content={<ChartTooltipContent labelKey="tier" nameKey="tier" indicator="dot" />}
                  />
                  <Pie
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {pipelineQuality.map((entry) => {
                const pct = totalLeads > 0 ? ((entry.count / totalLeads) * 100).toFixed(1) : "0";
                return (
                  <div key={entry.tier} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-tabular ml-auto font-medium">{pct}%</span>
                  </div>
                );
              })}
            </div>
          ...
          <CardHeader className="pb-2">
            <div className="flex flex-col gap-3">
              <div>
                ...
                <CardDescription>
                  {filteredCallList.length} leads &mdash; sorted by qualification score
                </CardDescription>
              </div>
                  ...
                  {filteredCallList.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className="py-8 text-center text-muted-foreground">
                        {callSearch || tierFilter !== "all" ? "No matching leads" : "No priority leads found"}
                      </TableCell>
                    </TableRow>
                  ) : (
                    filteredCallList.map((contact, i) => (
                      <TableRow key={`${contact.email}-${i}`} className="border-border/20 transition-colors hover:bg-muted/30">
                        ...
                        <TableCell className="text-center">
                          <span className="font-tabular text-sm font-bold text-foreground">{contact.totalScore}</span>
                        </TableCell>
                        <TableCell className="text-center">
                          <Badge variant="outline" className={`text-[10px] font-semibold ${tierBadgeStyles[contact.leadTier] || tierBadgeStyles["Tier D"]}`}>
                            {contact.leadTier}
                          </Badge>
                        </TableCell>
                      </TableRow>
