...
import type { SalesVelocityPoint, ProductPerformance, CollectionContact, DepositRecord } from "@/lib/types";
...
export function SalesCrm({ salesVelocity, topProducts, collectionList, deposits }: SalesCrmProps) {
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-3">
        <Card className="border-border/50 bg-card/80 backdrop-blur-sm lg:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-semibold">Sales Velocity</CardTitle>
            <CardDescription>Revenue over time by purchase date</CardDescription>
          </CardHeader>
          <CardContent className="px-2 sm:px-6">
            {salesVelocity.length === 0 ? (
              <div className="flex h-[220px] items-center justify-center text-sm text-muted-foreground sm:h-[280px]">No sales data available</div>
            ) : (
              <ChartContainer config={velocityConfig} className="h-[220px] w-full sm:h-[280px]">
                <AreaChart data={velocityData} accessibilityLayer>
                  ...
                  <XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fontSize: 10 }} tickMargin={8} interval="preserveStartEnd" />
                  <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 10 }} tickFormatter={(v) => `$${v}`} width={45} />
        ...
        <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
          ...
          <CardContent className="px-2 sm:px-6">
            {topProducts.length === 0 ? (
              <div className="flex h-[220px] items-center justify-center text-sm text-muted-foreground sm:h-[280px]">No product data available</div>
            ) : (
              <ChartContainer config={productsConfig} className="h-[220px] w-full sm:h-[280px]">
                <BarChart data={topProducts} layout="vertical" accessibilityLayer>
                  ...
                  <YAxis dataKey="product" type="category" tickLine={false} axisLine={false} tick={{ fontSize: 10 }} width={80} tickFormatter={(v: string) => (v.length > 12 ? v.slice(0, 12) + "…" : v)} />
      ...
      <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
        <CardHeader className="pb-2">
          <div className="flex flex-col gap-3">
            <div>
              <CardTitle className="text-base font-semibold">Deposits Ledger</CardTitle>
              <CardDescription>
                {filteredDeposits.length} records totaling{" "}
            ...
            <Select value={depositTypeFilter} onValueChange={setDepositTypeFilter}>
              <SelectTrigger className="h-8 w-full text-xs xs:w-[180px]">
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border/30 hover:bg-transparent">
                  <TableHead className="min-w-[100px] text-xs font-semibold uppercase tracking-wider text-muted-foreground">Contact</TableHead>
              ...
              <TableBody>
                {filteredDeposits.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={4} className="py-8 text-center text-muted-foreground">No deposits found</TableCell>
                  </TableRow>
                ) : (
                  filteredDeposits.map((dep, i) => (
                    <TableRow key={`dep-${i}`} className="border-border/20 transition-colors hover:bg-muted/30">
                      ...
                      <TableCell className="hidden text-muted-foreground sm:table-cell">
                        {dep.date ? new Date(dep.date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "—"}
                      </TableCell>
      ...
      <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
        <CardHeader className="pb-2">
          <div className="flex flex-col gap-3">
            <div>
              <CardTitle className="text-base font-semibold">The Collection List</CardTitle>
              <CardDescription>Contacts with outstanding balances &mdash; sorted highest to lowest</CardDescription>
            </div>
            <div className="relative w-full xs:w-auto">
              ...
              <input
                ...
                className="h-8 w-full rounded-md border border-border bg-background pl-8 pr-3 text-xs outline-none transition-colors placeholder:text-muted-foreground focus:border-ring focus:ring-1 focus:ring-ring xs:w-[240px]"
              />
            </div>
          </div>
        </CardHeader>
        <CardContent className="px-3 sm:px-6">
          <div className="max-h-[400px] overflow-auto rounded-md border border-border/30">
            <Table>
              <TableHeader>
                <TableRow className="border-border/30 hover:bg-transparent">
                  <TableHead className="min-w-[100px] text-xs font-semibold uppercase tracking-wider text-muted-foreground">Name</TableHead>
                  <TableHead className="hidden text-xs font-semibold uppercase tracking-wider text-muted-foreground sm:table-cell">Phone</TableHead>
                  ...
                  filteredCollection.map((contact, i) => (
                    <TableRow key={`${contact.email}-${i}`} className="border-border/20 transition-colors hover:bg-muted/30">
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground sm:hidden">{contact.phone}</div>
                      </TableCell>
